...
- `bootstrap_gtc.sh`: installs the latest released `gtc` via `cargo-binstall` and refreshes installable artifacts with `gtc install`. Every network step is wrapped in `retry()`, which re-attempts with jittered exponential backoff; tune with `BOOTSTRAP_RETRY_ATTEMPTS` (default `3`) and `BOOTSTRAP_RETRY_INITIAL_SECONDS` (default `5`).
- `tests/bootstrap_retry_test.sh`: regression tests for that `retry()` helper. Uses fake commands only -- no network, no cargo. Run by `ci/local_check.sh`.
...
- `generate_runtime_fixtures.sh`: creates the runtime startup bundle fixture with `gtc wizard --answers ...`, applies runtime setup with `gtc setup --no-ui --answers ...`, and packages the runtime `.gtbundle` artifact. The remaining limitation is the released `gtc start` behavior for the local-only WebChat runtime, not the wizard/setup generation flow.
